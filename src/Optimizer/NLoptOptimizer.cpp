...
namespace gosat {
	...
	int NLoptOptimizer::optimize (nlopt_func func, unsigned dim, double* x, double* min) const noexcept
		{
	...
	bool NLoptOptimizer::isSupportedLocalOptAlg(nlopt_algorithm local_opt_alg) noexcept
		{
			return (local_opt_alg == NLOPT_LN_BOBYQA
					|| local_opt_alg == NLOPT_LN_SBPLX);
		}

	bool NLoptOptimizer::isRequireLocalOptAlg(nlopt_algorithm opt_alg) noexcept
		{
			return opt_alg == NLOPT_G_MLSL || opt_alg == NLOPT_G_MLSL_LDS;
		}

	bool NLoptOptimizer::isSupportedGlobalOptAlg(nlopt_algorithm opt_alg) noexcept
		{
			switch (opt_alg) {
	...
	bool NLoptOptimizer::isRequirePopulation(nlopt_algorithm opt_alg) noexcept
	{
		switch (opt_alg) {
			case NLOPT_GN_MLSL:
			case NLOPT_GN_CRS2_LM:
	...
	int NLoptOptimizer::refineResult(nlopt_func func, unsigned dim, double* x, double* min)
	{
		nlopt_opt opt;
	...
	bool NLoptOptimizer::existsRoundingError(nlopt_func func, unsigned int dim, const double* x, const double* min) const noexcept
	{
		return func(dim, x, nullptr, nullptr) != *min;
	}

	void NLoptOptimizer::fixRoundingErrorNearZero(nlopt_func const func, unsigned dim, double* x, double* min) const noexcept
	{
	...
	double NLoptOptimizer::eval (nlopt_func func, unsigned dim, const double* x) const noexcept
	{
