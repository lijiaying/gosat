...
namespace gosat {
	...
	class NLoptOptimizer {
		public:
			...
			NLoptOptimizer(nlopt_algorithm global_alg, nlopt_algorithm local_alg = NLOPT_LN_BOBYQA);

			virtual ~NLoptOptimizer() = default;

			int optimize (nlopt_func func, unsigned dim, double* x, double* min) const noexcept;

			double eval (nlopt_func func, unsigned dim, const double* x) const noexcept;

			bool existsRoundingError (nlopt_func func, unsigned int dim, const double* x, const double* min) const noexcept;

			void fixRoundingErrorNearZero (nlopt_func const func, unsigned dim, double* x, double* min) const noexcept;
